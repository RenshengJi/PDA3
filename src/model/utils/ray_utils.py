...
"""Ray utilities for camera parameter estimation from ray maps."""
...
def get_extrinsic_from_camray(camray, conf, patch_size_y, patch_size_x, training=False):
    """
    Get camera extrinsic and intrinsic parameters from ray map.

    Args:
        camray: (B, S, num_patches_y, num_patches_x, 7) - ray map with direction and origin
        conf: (B, S, num_patches_y, num_patches_x, 1) - confidence map
        patch_size_y: Number of patches in y direction
        patch_size_x: Number of patches in x direction
        training: Whether in training mode (detaches gradients for RANSAC)

    Returns:
        pred_extrinsic: (B, S, 4, 4) - camera extrinsic matrix (w2c)
        pred_focal_lengths: (B, S, 2) - focal lengths
        pred_principal_points: (B, S, 2) - principal points
    """
